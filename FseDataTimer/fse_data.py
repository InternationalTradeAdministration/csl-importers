...
from shared import citizenship, convert_date, csl_meta, \
...
source_abbr = 'fse'
source_name = 'Foreign Sanctions Evaders (FSE) - Treasury Department'
source_list_url = 'https://www.treasury.gov/ofac/downloads/consolidated/consolidated.xml'
source_information_url = 'https://home.treasury.gov/policy-issues/financial-sanctions/consolidated-sanctions-list-non-sdn-lists/foreign-sanctions-evaders-fse-list'
...
null_fields = [
    'alt_names', 'call_sign', 'end_date', 'federal_register_notice',
...
def main():
    ...
    for entry in root.findall('xmlns:sdnEntry', ns):
        ...
        doc['programs'] = [p.text for p in programs]
        if 'FSE-' not in ' '.join(doc['programs']):
            continue
        ...
        doc['dates_of_birth'] = [convert_date.parse_date(dob.text)] if dob is not None else []
        ...
        doc['citizenships'] = '; '.join(doc['citizenships'])
        doc['dates_of_birth'] = '; '.join(doc['dates_of_birth'])
        doc['_id'] = doc['id']
        del doc['id']
        id_list = []
        for d in doc['ids']:
